import { parseHeaders, processHeaders, fulltenHeaders } from '../../src/helpers/headers'
...
describe('helpers:header', () => {
  ...
  describe('fulltenHeaders', () => {
    test('should flatten the headers and include common headers', () => {
      ...
      expect(fulltenHeaders(headers, 'get')).toEqual({
        Accept: 'application/json',
        'X-COMMON-HEADER': 'commonHeaderValue',
        'X-GET-HEADER': 'getHeaderValue'
      })
    ...
    test('should flatten the headers without common headers', () => {
      ...
      expect(fulltenHeaders(headers, 'patch')).toEqual({
        Accept: 'application/json'
      })
    })

    test('should do nothing if headers is undefined or null', () => {
      expect(fulltenHeaders(undefined, 'get')).toBeUndefined()
      expect(fulltenHeaders(null, 'post')).toBeNull()
    })
  })
})
